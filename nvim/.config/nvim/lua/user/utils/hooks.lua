...
function M.run_on_packer_complete()
  ...
  vim.g.colors_name = user.colorscheme
  pcall(vim.cmd, "colorscheme " .. user.colorscheme)
...
function M.reset_cache()
  ...
  for module, _ in pairs(package.loaded) do
    if module:match "user.core" or module:match "user.lsp" then
      package.loaded[module] = nil
      table.insert(user_modules, module)
    end
  end
  Log:trace(string.format("Cache invalidated for core modules: { %s }", table.concat(user_modules, ", ")))
...
function M.run_post_update()
  ...
  if vim.fn.has "nvim-0.7" ~= 1 then
    ...
    local ret = require_clean("user.utils.git").switch_user_branch(compat_tag)
    if ret then
