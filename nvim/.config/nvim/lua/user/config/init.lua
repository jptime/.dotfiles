...
function M:init()
  user = vim.deepcopy(require "user.config.defaults")

  require("user.keymappings").load_defaults()
  ...
  local user_lsp_config = require "user.lsp.config"
  user.lsp = vim.deepcopy(user_lsp_config)

  ---@deprecated replaced with lvim.builtin.alpha
  user.builtin.dashboard = {
    active = false,
    on_config_done = nil,
    search_handler = "",
...
local function handle_deprecated_settings()
  ...
  -- if user.autocommands.custom_groups then
  --   deprecation_notice(
...
function M:load(config_path)
  ...
  autocmds.define_autocmds(user.autocommands)
...
function M:reload()
  vim.schedule(function()
    require_clean("user.utils.hooks").run_pre_reload()

    M:load()

    require("user.core.autocmds").configure_format_on_save()

    local plugins = require "user.plugins"
    local plugin_loader = require "user.plugin-loader"

    plugin_loader.reload { plugins, user.plugins }
    require_clean("user.utils.hooks").run_post_reload()
  end)
end
